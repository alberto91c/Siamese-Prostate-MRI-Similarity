...
concl=concatenate([encoded_lhbv,encoded_lt2w], axis=-1)
concl=Dense(32,activation="sigmoid")(concl)
concr=concatenate([encoded_rhbv,encoded_rt2w], axis=-1)
...
siamese_net = Model(input=[left_inputt2w,right_inputt2w,left_inputhbv,right_inputhbv],output=distance)
adam = Adam(0.00001)
...
for j in range(0,epochs):
    ...
    for i in range(0,data.shape[0]//batch_size):
        ...
        c=siamese_net.train_on_batch([batch1t2w, batch2t2w, batch1hbv, batch2hbv], target_batch)
    ...
    loss_val, acc_val=siamese_net.evaluate([t2w[data_val[:,0]],t2w[data_val[:,1]],
                                                    hbv[data_val[:,0]],hbv[data_val[:,1]]], data_val[:,2],verbose=0)
    if loss_val<best_loss:
        best_loss=loss_val
        siamese_net.save('SiameseMax.h5')
...
# reload model
del siamese_net
output_model=load_model("SiameseMax.h5",custom_objects={'contrastive_loss': contrastive_loss})
...
np.save("output_DualMax.npy",output_model.predict([t2w[couple[:,0]],t2w[couple[:,1]],
